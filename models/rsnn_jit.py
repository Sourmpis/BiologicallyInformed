...
class RSNN(nn.Module):
    ...
    def forward(self, input, state, mem_noise, trial_noise, light):
        # type: (Tensor, List[Tensor], Tensor, Tensor, Tensor) -> Tuple[Tensor, Tensor,Tensor, List[Tensor]]
        spike_buffer, v, ref, b, jaw_buffer = (
            state[0],
            state[1],
            state[2],
            state[3],
        ...
        if self.latent_new:
            trial_noise_ready = self.lin1_tr_offset(
                torch.relu(self.lin_tr_offset(trial_noise))
            )
        else:
            trial_noise_ready = trial_noise @ self.trial_offset

        if self.trial_offset_bound:
            s = (self.thr0 - self.v_rest0) * 0.2
            trial_noise_ready[trial_noise_ready.abs() > s] = (
                s * trial_noise_ready[trial_noise_ready.abs() > s].sign()
            )
        ...
        mem_noise = mem_noise.clone() * self.bias
        spike_list = []
        voltage_list = []
        jaw_list = []
        z = spike_buffer[-1]
        for i in range(input.shape[0] // self.n_delay):
            ...
            if self.motor_areas.shape[0] > 0 and not self.jaw_open_loop:
                ...
                rec_cur_jaw = torch.zeros_like(rec_cur_exc)
    ...
    def step(self, rec_cur, inp_cur, z, v, ref, b, mem_noise, trial_noise_ready):
        # type: (List[Tensor], List[Tensor], Tensor, Tensor,Tensor, Tensor,Tensor, Tensor) -> Tuple[Tensor, Tensor,Tensor, Tensor, Tensor]
        ...
        thr_rest_diff = self.thr0 - self.v_rest
        if self.conductance_based:
            E_exc = self.v_rest0 + (thr_rest_diff) * 2
            E_inh = self.v_rest0 - (thr_rest_diff)
            exc_cur = (E_exc - v) * (inp_cur[0] + rec_cur[0]) / (E_exc - self.v_rest0)
            inh_cur = (E_inh - v) * (inp_cur[1] + rec_cur[1]) / -(E_inh - self.v_rest0)
        else:
        ...
        if self.train_noise_bias:
            mem_noise *= thr_rest_diff * self._sigma_mem_noise * self.dt**0.5
        else:
            mem_noise *= thr_rest_diff * self._sigma_mem_noise * self.dt**0.5
        v = (
            self.decay_v * v
            + (1 - self.decay_v) * (currents + trial_noise_ready + self.v_rest)
            + mem_noise
        )
        # Spike generation
        z = self.spike_function_bernoulli((v - thr) / (thr - self.v_rest0))
...
def zero_state(rsnn, batch_size):
    device = rsnn._w_rec.device
    spikes0 = torch.zeros(
        size=[rsnn.inter_delay, batch_size, rsnn.n_units], device=device
    )
    voltage0 = (
        torch.zeros(size=[1, batch_size, rsnn.n_units], device=device) + rsnn.v_rest
    )
    ref0 = torch.zeros(
        size=[1, batch_size, rsnn.n_units], device=device, dtype=torch.long
    )
    b0 = torch.zeros(size=[1, batch_size, rsnn.n_units], device=device)
    jaw_buffer0 = torch.zeros(
        size=[rsnn.jaw_delay + rsnn.n_delay - 1, batch_size, 1], device=device
    )
    state0 = (spikes0, voltage0, ref0, b0, jaw_buffer0)
    return state0


def reform_recurent(rsnn, lr, l1_decay=0.01):
    # here Deep-Rewiring is implemented
    for g in range(rsnn.rec_groups):
        weight, mask = reform_w_rec(
            rsnn,
            rsnn._w_rec[g].data.clone(),
            rsnn.mask_rec[g].data.clone(),
            lr,
            l1_norm_mult=l1_decay,
        )
        rsnn._w_rec[g].data.copy_(weight.data)
        rsnn.mask_rec[g].data.copy_(mask.data)
        rsnn.mask_rec.data.copy_(mask.data)
    reform_w_in(rsnn, lr, l1_norm_mult=l1_decay)
    rsnn.beta[rsnn.beta < 0] = 0


def reform_v_rest(rsnn, v_rest_bound=False):
    v = rsnn.v_rest.data
    v0 = rsnn.v_rest0
    t = rsnn.thr0
    # don't allow v_rest to be far away (20% of (t-v0)) from the initial value (v0)
    if v_rest_bound:
        rsnn.v_rest.data[v > v0 + (t - v0) * 0.2] = v0 + (t - v0) * 0.2
        rsnn.v_rest.data[v < v0 - (t - v0) * 0.2] = v0 - (t - v0) * 0.2
    else:
        rsnn.v_rest.data[v > t] = rsnn.thr0
        rsnn.v_rest.data[rsnn.v_rest < 2 * rsnn.v_rest0 - rsnn.thr0] = (
            2 * rsnn.v_rest0 - rsnn.thr0
        )


def reform_w_in(rsnn, lr, l1_norm_mult=0.01):
    w_in = rsnn._w_in.clone() - l1_norm_mult * lr
    w_in[w_in < 0] = 1e-12
    rsnn._w_in.data = w_in


def reform_w_jaw(rsnn, lr, l1_norm_mult=0.01):
    w_jaw = rsnn._w_jaw.clone() - l1_norm_mult * lr
    w_jaw[w_jaw < 0] = 1e-12
    rsnn._w_jaw.data = w_jaw


def reform_w_rec(
    rsnn, w_rec_orig, mask_rec_orig, lr, l1_norm_mult=0.01, extra_mask=None
):
    ...
    w_rec_orig.data[~(rsnn.mask_inter_inh_exc > 0)] = (
        w_rec_orig.data[~(rsnn.mask_inter_inh_exc > 0)] - lr
    )
    ...
    mask_rec_orig += rsnn.mask_self_exc
    mask_rec_orig += rsnn.block_connections
    mask_rec_orig += 1 - rsnn.mask_inter_inh_exc
    if extra_mask != None:
    ...
    for i, ((istart, jstart), (iend, jend)) in enumerate(rsnn.bounds_rec[0]):
        count_sub = (w_rec[istart:iend, jstart:jend] != 0).sum().float()
        # if true there are less than K dormant, here K is symbolic
        if not torch.eq(count_sub, rsnn.bounds_rec[1][i]):
            to_awake = (rsnn.bounds_rec[1][i] - count_sub).int()
...
def sample_trial_noise(rsnn, batch_size):
    device = rsnn.v_rest.device
    rsnn.trial_noise = torch.rand(batch_size, rsnn.latent_space, device=device)
    rsnn.trial_noise -= 0.5


def sample_mem_noise(rsnn, input_time, batch_size):
    device = rsnn.v_rest.device
    rsnn.mem_noise = torch.randn(input_time, batch_size, rsnn.n_units, device=device)
