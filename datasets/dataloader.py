...
class TrialDataset:
    ...
    def load_one_session(self, num_session, session):
        ...
        if type(trial_info.trial_type.values[0]) == str:
            trial_type_dict = {0: "Miss", 1: "Hit", 2: "CR", 3: "FA"}
            rev_tr_type_dict = {"Miss": 0, "Hit": 1, "CR": 2, "FA": 3}
        else:
            trial_type_dict = {i: i for i in self.trial_type}
            rev_tr_type_dict = trial_type_dict
        ...
        # check if the spikes are dense or sparse
        a = np.load(os.path.join(session_path, f"neuron_index_0.npy"))
        sparse_cond = (np.argsort(a) == np.arange(a.shape[0])).all()
        if sparse_cond:
            spike_times, spike_clusters = [], []
            for i, neuron in enumerate(actual_neurons):
                spike_times.append(
                    np.load(
                        os.path.join(session_path, f"neuron_index_{int(neuron)}.npy")
                    )
                )
                spike_clusters.append(np.ones_like(spike_times[-1]) * i)
            spike_times = (
                (np.concatenate(spike_times) / self.timestep).round().astype(int)
            )
            ...
            raster = csr_matrix(
                ...
                (spike_times.max() + 2, spike_clusters.max() + 1),
            )
        else:
            raster = []
            for i, neuron in enumerate(actual_neurons):
    ...
    def to_torch(self):
        ...
        for i, session in enumerate(sessions):
            ...
            indices = np.where(
                np.isin(self.data_dict["trial_types"][index], self.trial_type)
            )[0]
    ...
    def get_train_trial_type(self, train=0, device="cpu", jaw_tongue=1):
        ...
        max_trials = self.max_trials(train, trial_type=self.trial_type)
        ...
        for session in range(len(self.session_info[0])):
            ...
            indices_trial_type = np.isin(self.session_info[0][session], self.trial_type)
