...
import wandb
from models.rsnn import reform_recurent, reform_v_rest, sample_mem_noise
...
def train(opt, model, netD, optimizerG, optimizerD, step=-1):
    ...
    while step < opt.n_steps:
        ...
        optimizerG.step()
        with torch.no_grad():
            reform_recurent(model.rsnn, opt.lr, l1_decay=opt.l1_decay)
            reform_v_rest(model.rsnn)
        ...
        if (
            ...
            with torch.no_grad():
                ...
                sample_mem_noise(model.rsnn, model.T, input_spikes.shape[1])
                mem_noise = model.rsnn.mem_noise.clone()
                light = torch.zeros(
                    model.T, input_spikes.shape[1], model.rsnn.n_units, device=dev
                )
                model_spikes, voltages, model_jaw, state = model.step(
                    input_spikes, state, mem_noise, light
                )
