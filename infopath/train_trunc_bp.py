...
def train(opt, model, netD, optimizerG, optimizerD, step=-1):
    ...
    while step < opt.n_steps:
        ...
        model.rsnn.a *= 0.9996
        if model.rsnn.a < 0.14:
            model.rsnn.a = 0
        state, data_j, sp, jaw, dt = None, None, [], [], 1000
        torch.cuda.empty_cache()
        for t in range(0, model.T, dt):
            model_spikes, voltages, model_jaw, state = model(stims, state, t, t + dt)
            ...
            state = [st.detach() for st in state]
            sp.append(model_spikes)
            jaw.append(model_jaw)
        sp = torch.cat(sp, 0)
        jaw = torch.cat(jaw, 0)
        # Train Discriminator
        ...
        if opt.verbose:
            ...
            if step % (10 - (10 % len(stim_cond))) == 0:
                with torch.no_grad():
                    if data_jaw is not None:
                        data_jaw = data_jaw.to(opt.device)
                    t_trial_pearson_model = t_trial_pearson(
                        model.filter_fun1(data_spikes.to(opt.device)),
                        model.filter_fun1(sp),
                        model.filter_fun1(data_jaw),
                        model.filter_fun1(jaw),
                        model,
                        session_info,
                    )
        ...
        if (
            ...
            with torch.no_grad():
                ...
                if test_jaw is not None:
                    test_jaw = test_jaw.to(opt.device)
...
def explosion_reset(opt, life, step):
    ...
    life += 1
    if hasattr(model.rsnn, "a"):
        model.rsnn.a = 0.999**step
    if life > 3:
...
if __name__ == "__main__":
    ...
    if pars.config == "none":
        ...
        # opt = config_vahid()
        opt = get_opt(os.path.join("configs", "mechanism1_2areas"))
