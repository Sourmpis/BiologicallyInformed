...
from models.rsnn import RSNN, sample_mem_noise, sample_trial_noise, zero_state
...
class FullModel(nn.Module):
    def __init__(self, opt):
        ...
        self.rsnn = torch.jit.script(self.rsnn)
        ...
        if opt.geometric_loss:
            ...
            if loss_stats:
                ...
                self.trial_loss_fun = hard_trial_matching_loss_single
        # setup the loss splitter
        num_tasks = (
            opt.loss_trial_wise
            + opt.loss_neuron_wise
            + opt.loss_cross_corr
            + opt.loss_firing_rate
            + opt.loss_trial_matched_mle
        )
        # self.multi_task_splitter = NormalizedMultiTaskSplitter(num_tasks)
    ...
    @torch.no_grad()
    def steady_state(self, state=None):
        """
        ...
        if state is None:
            state = zero_state(self.rsnn, self.opt.batch_size)
        sample_mem_noise(self.rsnn, spike_data.shape[0], self.opt.batch_size)
        sample_trial_noise(self.rsnn, self.opt.batch_size)
        dev = self.opt.device
        light = torch.zeros(
            spike_data.shape[0], spike_data.shape[1], self.rsnn.n_units, device=dev
        )
        _, _, _, state = self.rsnn(
            spike_data, state, self.rsnn.mem_noise, self.rsnn.trial_noise, light
        )
        return state

    def step(self, input_spikes, state, mem_noise, light):
        opt = self.opt
        self.rsnn.mem_noise = mem_noise
        spike_outputs, voltages, model_jaw, state = self.rsnn(
            input_spikes.to(opt.device),
            state,
            self.rsnn.mem_noise,
            self.rsnn.trial_noise,
            light,
        )
    ...
    def forward(self, stims, light=None):
        self.opt.batch_size = stims.shape[0]
        state = self.steady_state()
        input_spikes = self.input_spikes(stims)
        sample_mem_noise(self.rsnn, self.T, stims.shape[0])
        mem_noise = self.rsnn.mem_noise.clone()
        dev = self.opt.device
        light = torch.zeros(
            input_spikes.shape[0], input_spikes.shape[1], self.rsnn.n_units, device=dev
        )
        return self.step(input_spikes, state, mem_noise, light)
